...
const AppleTv = (props) => {
  ...
  const appleTVOriginals = useSelector(selectAppleTVOriginals);
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    dispatch(fetchAppleTVOriginals());
  }, [dispatch]);

  const categoryConfig = {
    id: 'apple-originals',
    title: 'Apple TV+ Originals',
    route: '/more/apple-originals' 
  };

  const moviesPerSlide = 8;
  const validMovies =
    appleTVOriginals.results?.filter((movie) => movie.poster_path) || [];
  ...
  return (
    <Container>
      <Header>
        <Title>
          <h3 className="TITLE">{categoryConfig.title}</h3>
        </Title>
        ...
        <Controls>
          ...
          {currentIndex === totalSlides - 1 ? (
            <MoreButton to={categoryConfig.route}>More ›</MoreButton>
          ) : (
            <NavButton
              onClick={nextSlide}
