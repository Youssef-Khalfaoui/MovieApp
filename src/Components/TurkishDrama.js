...
const TurkishDrama = (props) => {
  const dispatch = useDispatch();
  const trendingMovies = useSelector(selectLatestTurkishMovies);
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    dispatch(fetchLatestTurkishMovies());
  }, [dispatch]);
const categoryConfig = {
  id: 'latest-turkish-movies',
  title: 'Latest Turkish Movies',
  route: '/more/latest-turkish-movies'
};
  ...
  return (
    <Container>
      <Header>
        <Title>
          <h3 className="TITLE">Turkish Drama </h3>
        </Title>
