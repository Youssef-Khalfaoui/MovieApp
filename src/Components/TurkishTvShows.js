...
const TurkishTVShows = (props) => {
  const dispatch = useDispatch();
  const trendingMovies = useSelector(selectNewestTurkishTV);
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    dispatch(fetchNewestTurkishTV());
  }, [dispatch]);

const categoryConfig = {
  id: 'newest-turkish-tv',
  title: 'Newest Turkish TV',
  route: '/more/newest-turkish-tv'
};
  ...
  return (
    <Container>
      <Header>
        <Title>
          <h3 className="TITLE">Turkish TV Shows </h3>
        </Title>
