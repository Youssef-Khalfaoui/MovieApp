...
const FrenchDubbed = (props) => {
  const dispatch = useDispatch();
  const trendingMovies = useSelector(selectLatestFrenchContent);
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    dispatch(fetchLatestFrenchContent());
  }, [dispatch]);
...
const categoryConfig = {
  id: 'french-content',
  title: 'French Movies',
  route: '/more/french-content'
};
  return (
    <Container>
      <Header>
        <Title>
          <h3 className="TITLE">✨ French Dubbed ✨</h3>
        </Title>
